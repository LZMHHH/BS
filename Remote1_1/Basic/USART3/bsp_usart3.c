...
static void NVIC_Configuration(uint8_t priority)
{
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	NVIC_InitStructure.NVIC_IRQChannel = DEBUG3_UART_Tx_DMA_IRQ;   // 发送DMA通道的中断配置 
  ...
  /* 配置USART为中断源 */
  NVIC_InitStructure.NVIC_IRQChannel = DEBUG3_USART_IRQ;         //串口中断配置
  /* 抢断优先级*/
...
void DEBUG3_UART_Tx_DMA_IRQHandler(void)
{
    ...
    {
			  uart3_sending = 0;     //空闲
        DMA_ClearFlag(DEBUG3_UART_Tx_DMA_FLAG);         // 清除标志
			  DMA_Cmd(DEBUG3_UART_Tx_DMA_Channel, DISABLE);   // 关闭DMA通道   
...
void DEBUG3_USART_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(DEBUG3_USARTx, USART_IT_IDLE) != RESET)  // 空闲中断
    {
			  USART_ClearITPendingBit( DEBUG3_USARTx, USART_IT_IDLE );  //清除空闲中断
			  USART_ReceiveData(DEBUG3_USARTx); //读DR,只有读过一次，才能真正清除标志
			  USART_ClearFlag( DEBUG3_USARTx, USART_FLAG_IDLE );  //读SR其实就是清除标志
			
			  DMA_Cmd(DEBUG3_UART_Rx_DMA_Channel, DISABLE);       // 关闭DMA ，防止干扰
			  DMA_ClearFlag( DEBUG3_UART_Rx_DMA_FLAG );           // 清DMA标志位
			  num = DEBUG3_RX_BSIZE - DMA_GetCurrDataCounter(DEBUG3_UART_Rx_DMA_Channel); //获得接收到的字节数
			  DEBUG3_UART_Rx_DMA_Channel->CNDTR = DEBUG3_RX_BSIZE;    //  重新赋值计数值，必须大于等于最大可能接收到的数据帧数目
...
#if DEBUG3_SAFETY
				xQueueSendFromISR(  xQueue_uart3Rx,               /* 消息队列的句柄 */
