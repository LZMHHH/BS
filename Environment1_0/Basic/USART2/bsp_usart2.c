...
static void NVIC_Configuration(uint8_t priority)
{
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	NVIC_InitStructure.NVIC_IRQChannel = DEBUG2_UART_Tx_DMA_IRQ;   // 发送DMA通道的中断配置 
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x07;      // 优先级设置
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x00;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
  
  /* 配置USART为中断源 */
  NVIC_InitStructure.NVIC_IRQChannel = DEBUG2_USART_IRQ;         //串口中断配置
  /* 抢断优先级*/
...
/**
  * @brief  USARTx TX DMA 配置，内存到外设(UART2->DR)
  * @param  无
  * @retval 无
  */
void UART2_DMA_Config(void)
...
void DEBUG2_UART_Tx_DMA_IRQHandler(void)
{
    ...
    {
			  uart2_sending = 0;     //空闲
        DMA_ClearFlag(DEBUG2_UART_Tx_DMA_FLAG);         // 清除标志
			  DMA_Cmd(DEBUG2_UART_Tx_DMA_Channel, DISABLE);   // 关闭DMA通道   
...
void DEBUG2_USART_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(DEBUG2_USARTx, USART_IT_IDLE) != RESET)  // 空闲中断
    {
			  USART_ClearITPendingBit( DEBUG2_USARTx, USART_IT_IDLE );  //清除空闲中断
			  USART_ReceiveData(DEBUG2_USARTx); //读DR,只有读过一次，才能真正清除标志
			  USART_ClearFlag( DEBUG2_USARTx, USART_FLAG_IDLE );  //读SR其实就是清除标志
			
			  DMA_Cmd(DEBUG2_UART_Rx_DMA_Channel, DISABLE);       // 关闭DMA ，防止干扰
			  DMA_ClearFlag( DEBUG2_UART_Rx_DMA_FLAG );           // 清DMA标志位
			  num = DEBUG2_RX_BSIZE - DMA_GetCurrDataCounter(DEBUG2_UART_Rx_DMA_Channel); //获得接收到的字节数
			  DEBUG2_UART_Rx_DMA_Channel->CNDTR = DEBUG2_RX_BSIZE;    //  重新赋值计数值，必须大于等于最大可能接收到的数据帧数目
			  

			  //Flage_StateRx = State_RTno;     //接收完成
			  //屏蔽这个的意思是处理处理完本次数据再开启接收
			  //DMA_Cmd(DEBUG2_UART_Rx_DMA_Channel, ENABLE);        //DMA 开启，等待数据。注意，如果中断发送数据帧的速率很快，MCU来不及处理此次接收到的数据，中断又发来数据的话，这里不能开启，否则数据会被覆盖。有2种方式解决。
			
#if DEBUG2_SAFETY
				xQueueSendFromISR(  xQueue_uart2Rx,               /* 消息队列的句柄 */
