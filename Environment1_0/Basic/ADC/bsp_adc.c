...
void Adc_Gpio_Init(void)
{
	...
	//PC2 作为模拟通道输入引脚                         
	GPIO_InitStructure.GPIO_Pin = ADC_GASA_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
	GPIO_Init(ADC_GASA_PORT, &GPIO_InitStructure);	
	//PC3 作为模拟通道输入引脚                         
	GPIO_InitStructure.GPIO_Pin = ADC_GASB_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
	GPIO_Init(ADC_GASB_PORT, &GPIO_InitStructure);	
	//PC4 作为模拟通道输入引脚                         
	GPIO_InitStructure.GPIO_Pin = ADC_GASC_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
	GPIO_Init(ADC_GASC_PORT, &GPIO_InitStructure);	
...
void Adc_Adc_Init(void)
{
	...
	ADC_RegularChannelConfig(ADCx, ADC_GASA_CHANNEL,  ADC_GASA+1, ADC_SampleTime_239Cycles5);	//采样个数ADC_SAMPLE_NUM
	ADC_RegularChannelConfig(ADCx, ADC_GASB_CHANNEL, ADC_GASB+1, ADC_SampleTime_239Cycles5);	//总共耗时4*21*ADC_SAMPLE_NUM（64）=5.4ms<10ms
	ADC_RegularChannelConfig(ADCx, ADC_GASC_CHANNEL,  ADC_GASC+1, ADC_SampleTime_239Cycles5);
